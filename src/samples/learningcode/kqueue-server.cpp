...
void directWrite(int efd, int fd) {
    printf("%s\n", __func__);
	char buf[4096] = {0};
	snprintf(buf, 4096, "Hi Deep, Copy!");
	printf("send: %s\n", buf);
	int n = strlen(buf);
...
void handleRead(int efd, int fd) {
    ...
    //	updateEvents(efd, fd, kReadEvent, false);
    //    return;
	//}
...
int main() {
    ...
    updateEvents(kqfd, listenfd, kReadEvent, false);
    while(true) { //实际应用应当注册信号处理函数，退出时清理资源
        loopEvents(kqfd, listenfd, 10000);
    }
    return 0;
}
